...
for i in range(N_patients-2,-1,-1):
    ...
    if (i>1):
        four=P[3-1,i-1]+P[1-1,i+1]>=P[2-1,i]+P[3-1,i] and P[3-1,i-1]>=P[3-1,i]
    else:
        four=3<2

    if (one and two and not four):
    ...
    elif (one and two and four) :
        OR[i]=1
        C[1-1,i]=C[1-1,i+1]-P[1-1,i+1]-P[3-1,i-1]
...
Plot_gantt(C,P,OR,pos,'prova_back_mod_.png')